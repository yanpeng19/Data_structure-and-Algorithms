...
public:
	...
	vec(vector<T> v)
	{
		...
		while (r<i)
		{
			alloc.construct(first_free++, v[r++]);
		}
	}
...
/*
堆排序：
1.将hi和lo中间的数据整理成 一个备份的完全二叉堆
2.将备份二叉堆反复delete_Max,获取最大数并赋予this[hi]
3.hi--
*/
template<typename T>
void vec<T>::heap_sort(int lo, int hi)
{
	if (empty()) return;
	PQ_ComplHead<T> H(element + lo, lo + hi);
	while(hi-lo>=0)
		*(element + hi--) = H.delete_Max();
}

template<typename T>
void vec<T>::heap_sort()
{
	if (empty()) return;
	int lo = 0;
	int hi = size()-1;
...
void vec<T>::insert(unsigned rank, const T& t)
{
	...
	//else if (rank == size()&&!empty()) push_back(t);
	//else if (empty() && rank == 0) push_back(t);
	//else this->operator[](rank) = t;
